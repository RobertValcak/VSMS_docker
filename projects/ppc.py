...
def pcc_forwardKinematics(kappa, phi, L, num_point=50):
    if abs(kappa) < 1e-6:
        s= np.linspace(0, L, num_point)
        x = np.zeros_like(s)
        z=s
    else:
        s = np.linspace(0, L, num_point)
...
def plot_robot(L1, L2, L3):
    ...
    X,Y,Z = pcc_forwardKinematics(kappa, phi, L)

    ax.plot(X,Y,Z, linewidth=4)
    ax.scatter(X[-1], Y[-1],Y[-1], color='red', label='Tip')

    ax.set_title(f"L1={L1: .2f},L1={L2: .2f},L1={L3: .2f} ")
    ax.legend()
    plt.show()
